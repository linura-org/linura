...
fn finish_durable_command(result: Result<(), String>) -> ExitCode {
    match result {
        ...
        Err(_) => {
            ...
            // machine/session identity. Do not reflect the raw error into stderr:
            // production logs are a lower-trust boundary than the local durable
            // state they diagnose. Operators get a stable failure class while
            // recovery proceeds from the protected state/evidence itself.
            eprintln!("durable First Boot failed (protected diagnostic details withheld)");
            ExitCode::FAILURE
        }
    }
}
