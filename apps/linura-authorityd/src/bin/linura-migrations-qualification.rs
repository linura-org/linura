...
use linura_migrations::{
    ...
    V09PersistentStateMigration, ValidatedBackup, create_validated_sqlite_backup,
};
...
fn run() -> Result<(), String> {
    ...
    let authority_wal = sqlite_sidecar_path(&authority, "-wal");
    let wal_before = fs::metadata(&authority_wal).map_err(io_string)?.len();
    if wal_before == 0 {
        return Err("authority qualification fixture did not retain committed WAL payload".into());
    }
    ...
    let library_evidence =
        create_validated_sqlite_backup(&library, &library_backup).map_err(display_string)?;
    let authority_evidence =
        create_validated_sqlite_backup(&authority, &authority_backup).map_err(display_string)?;
    ...
    println!("q11_sqlite_backup=wal-checkpointed-recovery-unit");
