...
16. **Many inputs, one authority architecture.** GUI, config, keyboard, quick settings, CLI, automation and agents may differ in interaction and risk-appropriate ceremony, but Linura-managed durable external mutations converge on the same typed Control-owned authority semantics and canonical lifecycle. Lighter non-durable interactions use proportionate typed paths; no interface becomes a privilege bypass.
17. **Classify before authority.** Operation semantics and trusted risk determine the proportionate path: ephemeral experience, authoritative query, Linura-owned state, bounded transient external effect, or full managed external effect. Unknown effects fail closed; convenience never downgrades authority.
