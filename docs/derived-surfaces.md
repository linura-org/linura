...
The command palette remains navigation-only: its catalog contains explicit trusted presentation targets and may consume plain descriptors projected by bounded shell-integration adapters such as the qualified Hyprland workspace controller. Provider objects and compositor-control APIs do not enter the presentation component. Search, selection, opening/closing surfaces and workspace switching are `ExperienceEphemeral`; the palette emits typed navigation intent and the shell adapter resolves the exact live workspace before using the typed activation API rather than raw compositor command strings. Effectful palette entries must resolve through typed controller boundaries to registered Linura operations; the palette never assigns `OperationClass`, risk or approval semantics.
