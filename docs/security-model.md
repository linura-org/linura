...
`contracts/components.toml` additionally marks agent components as proposal-only future surfaces until their roadmap milestones. Workspace presence is not authority. See [ADR 0026](adr/0026-component-maturity-and-milestone-activation.md).
...
v0.6 is the first candidate to integrate the complete eleven-stage lifecycle for one bounded Experimental effect. The exact process, identity, approval, recovery and non-goal decisions are recorded in [ADR 0025](adr/0025-bounded-v0.6-managed-mutation-authority.md).
