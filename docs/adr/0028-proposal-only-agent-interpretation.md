...
10. while that durable guard is held, revalidate the complete current context/freshness/capability/decision binding using trusted Control time, compute the earliest required observation-freshness deadline, and fail closed if any authority generation, target revision, freshness requirement or decision applicability drifted;
11. mint a transaction-scoped sealed acceptance-commit capability exact-bound to the acceptance material, durable transaction identity, authority generation and freshness deadline;
12. consume that capability through the authority-internal Library acceptance primitive without releasing or reacquiring the durable guard; immediately at the intent+acceptance-record write/CAS linearization, mechanically re-check the sealed freshness deadline using trusted Control time and abort/roll back if it has been crossed; and
