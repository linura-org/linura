...
Untrusted runtime/adapter logic owns only provider-specific, non-side-effecting protocol adaptation such as request encoding, response decoding and bounded stream parsing. Before any provider I/O, it produces a bounded, credential-free `PreparedProviderInvocation`; Linura Control validates that envelope against the selected provider/adapter/endpoint profile and attempt bounds and records its canonical digest. Runtime/adapter logic runs without ambient network/process/session transport access and without raw or reusable provider credentials. Every side-effecting hosted or local provider call crosses a trusted provider-invocation gate outside that sandbox. The gate is a bounded mechanism rather than an orchestration owner: it may resolve the exact admitted credential/endpoint and carry one invocation, but it cannot select providers, admit retries, widen budgets/deadlines, initiate fallback/advisors or aggregate results.
...
For each admission, Linura Control mints an unforgeable, single-use `ProviderInvocationPermit` exact-bound to the attempt identity, selected adapter/provider and endpoint class, canonical interpretation-request digest, exact `PreparedProviderInvocation` digest, network/offline classification, per-attempt bounds and aggregate-orchestration generation/deadline/budget. The permit is conveyed only to the trusted invocation gate through its protected capability channel; it is absent from public, runtime, adapter and provider payload schemas and cannot be minted, cloned, replayed or reconstructed by those callers.

The invocation gate verifies the exact prepared-invocation bytes/profile and atomically consumes the permit before resolving credentials or initiating authentication, DNS, socket, process/session or provider transport. A substituted invocation or consumed, forged, mismatched, expired or already-used permit fails before any such side effect. The runtime/adapter sandbox has no alternate provider egress or credential path, so even compromised adapter logic cannot issue a second request directly. One permit authorizes at most one side-effecting provider invocation and is consumed regardless of success, timeout, rate limit, transport failure, cancellation or partial response. Transport-library automatic retries, redirects that initiate another request, authentication-challenge replay, uncertain-write replay and stream reconnect/resume are disabled; each would be another invocation requiring fresh Control admission.

Adapter logic must return those bounded outcomes to Control without internally retrying, reissuing or starting a replacement/resume request. Every repeated provider invocation requires a fresh Control admission and fresh permit against the still-authoritative aggregate budget and deadline. A provider receives no implicit retry authority. Provider failure, timeout, cancellation, malformed output or budget exhaustion produces no authoritative intent mutation.
...
- `linura-provider-sdk` owns provider-neutral `PreparedProviderInvocation`, single-invocation adapter and invocation-gate ports, not retry admission, model authority or orchestration policy. Concrete composition must isolate untrusted adapter logic from ambient provider egress/credentials and place side-effecting transport behind the permit-consuming trusted gate.
