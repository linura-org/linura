...
- immediately at the actual intent+acceptance-record write/CAS, the Library path rechecks trusted Control time and permits persistence only when `time_floor <= now < deadline`; rollback/reset/unknown time continuity, invalid/missing normalization evidence or deadline expiry rolls the transaction back;
...
- qualification separately injects inclusive-observation endpoint normalization, decision/approval exact-expiry normalization, normalization overflow/unknown-semantics failure, post-mint observation expiry, post-mint decision/approval expiry and post-mint trusted-time rollback so one case cannot mask another.
