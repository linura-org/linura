...
fn systemd_unit_file_boot_enabled(state: &str) -> bool {
    matches!(state, "enabled" | "enabled-runtime")
}

fn systemd_unit_is_uninstantiated_template(unit: &str) -> bool {
...
fn systemd_candidate_units() -> Result<std::collections::BTreeSet<String>, DurableBootstrapError> {
    ...
    for line in String::from_utf8_lossy(&unit_files.stdout).lines() {
        ...
        // `list-unit-files` includes uninstantiated template definitions such as
        // `serial-getty@.service`. They are definitions, not runnable unit
        // instances, and `systemctl show template@.service` is intentionally
        // invalid. Concrete enabled instances (for example `foo@bar.service`)
        // and all active runtime instances remain authoritative candidates.
        if systemd_unit_file_boot_enabled(state)
            && !systemd_unit_is_uninstantiated_template(unit)
        {
...
fn ssh_unit_exposes_runtime_or_boot(properties: &str) -> bool {
    properties.lines().any(|line| {
        matches!(
            ...
            .strip_prefix("UnitFileState=")
            .is_some_and(systemd_unit_file_boot_enabled)
    })
}
...
fn ssh_listener_running() -> Result<bool, DurableBootstrapError> {
    let mut listeners = tcp_listener_inodes(Path::new("/proc/net/tcp"), false)?;
    listeners.extend(tcp_listener_inodes(Path::new("/proc/net/tcp6"), true)?);
...
fn ssh_enabled_once() -> Result<bool, DurableBootstrapError> {
    for unit in systemd_candidate_units()? {
        ...
        if !output.status.success() {
            // A transient unit can disappear between the authoritative
            // enumeration and property read. Ignore it only after a fresh
            // authoritative snapshot proves it is no longer active or
            // boot-enabled; a persistent candidate remains fail-closed.
            if systemd_candidate_units()?.contains(&unit) {
                return Err(DurableBootstrapError::InvalidVerificationEvidence(
...
const SSH_OBSERVATION_ATTEMPTS: usize = 4;

fn retry_ssh_observation<F>(mut observe: F) -> Result<bool, DurableBootstrapError>
where
    F: FnMut() -> Result<bool, DurableBootstrapError>,
{
    let mut last_error = None;
    for attempt in 0..SSH_OBSERVATION_ATTEMPTS {
        match observe() {
            Ok(exposed) => return Ok(exposed),
            Err(error) => {
                last_error = Some(error);
                if attempt + 1 < SSH_OBSERVATION_ATTEMPTS {
    ...
    match last_error {
        ...
        None => Err(DurableBootstrapError::InvalidVerificationEvidence(
            "SSH exposure observation did not execute",
        )),
    }
}

fn ssh_enabled() -> Result<bool, DurableBootstrapError> {
    retry_ssh_observation(ssh_enabled_once)
}
...
impl DurableBootstrapCoordinator {
    ...
    let ssh_exposed = ssh_enabled().map_err(|_| {
        DurableBootstrapError::InvalidVerificationEvidence(
            "Q8 security baseline could not observe SSH exposure",
        )
    })?;
    if ssh_exposed {
        return Err(DurableBootstrapError::InvalidVerificationEvidence(
            "Q8 security baseline rejected SSH exposure",
        ));
    }
    let firewall_fail_closed = firewall_default_deny().map_err(|_| {
        DurableBootstrapError::InvalidVerificationEvidence(
            "Q8 security baseline could not observe firewall policy",
        )
    })?;
    if !firewall_fail_closed {
        return Err(DurableBootstrapError::InvalidVerificationEvidence(
            "Q8 security baseline requires inbound default-deny firewall",
        ));
    }
    let apt_fail_closed = apt_sources_fail_closed().map_err(|_| {
        DurableBootstrapError::InvalidVerificationEvidence(
            "Q8 security baseline could not observe APT source policy",
        )
    })?;
    if !apt_fail_closed {
        return Err(DurableBootstrapError::InvalidVerificationEvidence(
            "Q8 security baseline rejected insecure APT source override",
        ));
    }
...
mod effect_verifier_tests {
    ...
    #[test]
    fn masked_ssh_unit_is_not_runtime_or_boot_exposure() {
        let masked = concat!(
            "Id=ssh.service\n",
        ...
        assert!(ssh_serving_unit(masked));
        assert!(!ssh_unit_exposes_runtime_or_boot(masked));

        let enabled = concat!(
            "Id=custom-remote.service\n",
            "Description=OpenSSH server daemon\n",
            "ActiveState=inactive\n",
            "UnitFileState=enabled\n",
        );
        assert!(ssh_serving_unit(enabled));
        assert!(ssh_unit_exposes_runtime_or_boot(enabled));
    }

    #[test]
    fn uninstantiated_systemd_templates_are_not_runtime_instances() {
        assert!(systemd_unit_is_uninstantiated_template("serial-getty@.service"));
        assert!(systemd_unit_is_uninstantiated_template("example@.socket"));
        assert!(!systemd_unit_is_uninstantiated_template(
            "serial-getty@ttyS0.service"
        ));
        assert!(!systemd_unit_is_uninstantiated_template("ssh.service"));
    }

    #[test]
    fn only_actual_enablement_states_are_boot_exposure() {
        for state in [
            "alias",
            "disabled",
            "generated",
            ...
            assert!(
                !systemd_unit_file_boot_enabled(state),
                "{state} must not be treated as boot enablement"
            );
            let properties = format!(
                "Id=custom-remote.service\nDescription=OpenSSH server daemon\nActiveState=inactive\nUnitFileState={state}\n"
            );
            assert!(
                !ssh_unit_exposes_runtime_or_boot(&properties),
                "inactive {state} unit must not be treated as SSH exposure"
            );
        }

        for state in ["enabled", "enabled-runtime"] {
            assert!(systemd_unit_file_boot_enabled(state));
            let properties = format!(
                "Id=custom-remote.service\nDescription=OpenSSH server daemon\nActiveState=inactive\nUnitFileState={state}\n"
            );
            assert!(ssh_unit_exposes_runtime_or_boot(&properties));
        }

        let active_alias = concat!(
            "Id=custom-remote.service\n",
            "Description=OpenSSH server daemon\n",
            "ActiveState=active\n",
            "UnitFileState=alias\n",
        );
        assert!(ssh_unit_exposes_runtime_or_boot(active_alias));
    }

    #[test]
    fn ssh_observation_retry_accepts_only_a_successful_reobservation() {
        let mut attempts = 0_u8;
        let exposed = retry_ssh_observation(|| {
            attempts = attempts.saturating_add(1);
            if attempts == 1 {
                return Err(DurableBootstrapError::InvalidVerificationEvidence(
                    "transient SSH observation race",
                ));
            }
            Ok(false)
        })
