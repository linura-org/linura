fn parse_state(bytes: &[u8]) -> Result<DurableBootstrapState, DurableBootstrapError> {
    ...
    let first_boot_plan_sha256 = match state_field(lines.next(), "first_boot_plan_sha256")? {
        "-" => None,
        value
            if value.len() == 64
        ...
        _ => {
            return Err(DurableBootstrapError::CorruptState(
                "invalid First Boot plan SHA-256 binding".into(),
            ));
        }
    };
    let first_boot_plan_count = state_field(lines.next(), "first_boot_plan_count")?
        .parse::<u32>()
        .map_err(|_| {
            DurableBootstrapError::CorruptState(
                "invalid First Boot plan count".into(),
            )
        })?;
