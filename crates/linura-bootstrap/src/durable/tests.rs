...
mod tests {
    ...
    fn test_authority_key(path: &Path) -> PathBuf {
        let key_path = path
            .parent()
            .unwrap_or_else(|| unreachable!("receipt has no parent"))
            .join("control-receipt-auth.key");
        if !key_path.exists() {
            fs::write(&key_path, [0x5a_u8; OWNER_ENROLLMENT_AUTH_KEY_BYTES])
                .unwrap_or_else(|error| unreachable!("{error}"));
    ...
    fn trusted_owner_receipt(
        ...
        let key_path = test_authority_key(path);
        let signer = OwnerEnrollmentAuthoritySigner::open(&key_path)
            .unwrap_or_else(|error| unreachable!("{error}"));
        let bytes = signer
        ...
        fs::set_permissions(path, fs::Permissions::from_mode(0o600))
            .unwrap_or_else(|error| unreachable!("{error}"));
        let verifier = OwnerEnrollmentAuthorityVerifier::open(&key_path)
            .unwrap_or_else(|error| unreachable!("{error}"));
        TrustedOwnerEnrollmentReceipt::load(path, session_id, machine_id, &verifier)
            .unwrap_or_else(|error| unreachable!("{error}"))
    }

    fn qualification_revocation(
        coordinator: &DurableBootstrapCoordinator,
    ) -> TrustedPreparerAuthorityRevocationReceipt {
        TrustedPreparerAuthorityRevocationReceipt::qualification_observation(
            coordinator.state(),
    ...
    fn complete_stage(
        ...
        if stage_requires_effect_boundary(stage) {
            ...
            if stage == BootstrapStage::FirstBootPlanning {
                coordinator
                    .bind_first_boot_plan("ab".repeat(32), 1)
                    .unwrap_or_else(|error| unreachable!("{error}"));
            }
            coordinator
    ...
    fn prepare_for_another_owner_persists_stable_pending_then_mints_fresh_owner_evidence() {
        ...
        let revocation = qualification_revocation(&coordinator);
        coordinator
    ...
    fn unattended_local_requires_exact_bound_manifest_and_stable_pending() {
        ...
        assert_eq!(coordinator.state().provisioning().manifest_id(), Some("manifest-1"));
        assert_eq!(
            coordinator.state().provisioning().manifest_connectivity(),
            Some(BootstrapConnectivity::Offline)
        );
        assert_eq!(coordinator.state().provisioning().manifest_defer_owner_enrollment(), Some(true));
        assert_eq!(coordinator.state().provisioning().manifest_source_ref(), Some("local:library"));
        assert_eq!(coordinator.state().provisioning().manifest_setup_ref(), Some("setup:v1"));
        assert_eq!(coordinator.state().provisioning().manifest_machine_profile_ref(), Some("profile:v1"));
        assert_eq!(coordinator.state().provisioning().manifest_host_identity(), Some("host:test"));
        advance_to_owner_resolution(&mut coordinator);
        let revocation = qualification_revocation(&coordinator);
        coordinator
            .enter_owner_enrollment_pending(&revocation)
        ...
        assert_eq!(reopened.state().provisioning().manifest_id(), Some("manifest-1"));
        assert_eq!(reopened.state().provisioning().manifest_connectivity(), Some(BootstrapConnectivity::Offline));
        assert_eq!(reopened.state().provisioning().manifest_defer_owner_enrollment(), Some(true));
        assert_eq!(reopened.state().provisioning().manifest_source_ref(), Some("local:library"));
        assert_eq!(reopened.state().provisioning().manifest_setup_ref(), Some("setup:v1"));
        assert_eq!(reopened.state().provisioning().manifest_machine_profile_ref(), Some("profile:v1"));
        assert_eq!(reopened.state().provisioning().manifest_host_identity(), Some("host:test"));
    }
