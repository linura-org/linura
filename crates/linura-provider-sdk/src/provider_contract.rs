...
mod tests {
    ...
    fn durable_generation_zero_is_valid_executor_correlation_material() {
        let effect = effect("linura-v05-qualification-a.service");
        ...
        assert_eq!(binding.validate_for(&effect), Ok(()));
    }

    #[test]
    fn execution_binding_rejects_effect_substitution() {
        let effect_a = effect("linura-v05-qualification-a.service");
        let effect_b = effect("linura-v05-qualification-b.service");
        let binding = id(ExecutionBinding::new(
            "tx:test",
            1,
            2,
        ...
        assert_eq!(
            binding.validate_for(&effect_b),
            Err(ComponentContractError::BindingMismatch)
        );
    }

    #[test]
    fn execution_binding_rejects_authority_use_substitution() {
        let effect = effect("linura-v05-qualification-a.service");
        let binding = id(ExecutionBinding::new(
            "tx:test",
            1,
        ...
        let substituted = id(ExecutionBinding::new(
            "tx:test",
            1,
            2,
            digest(1),
        ...
        assert_ne!(binding.dispatch_digest, substituted.dispatch_digest);
    }
}
