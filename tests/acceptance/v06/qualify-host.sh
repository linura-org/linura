...
if [[ "$guest_rc" -ne 0 ]]; then
  {
    ...
    ssh "${ssh_common[@]}" -p "$ssh_port" linura@127.0.0.1 \
      'sudo -n ls -la /var/lib/linura-authority || true; sudo -n sqlite3 /var/lib/linura-authority/authority.sqlite3 "PRAGMA journal_mode; PRAGMA integrity_check;" || true; sudo -n sqlite3 /var/lib/linura-authority/authority.sqlite3 ".tables" || true'
    echo '--- dispatch log ---'
    ssh "${ssh_common[@]}" -p "$ssh_port" linura@127.0.0.1 \
