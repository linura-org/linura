...
Status: `v0.6.0` released — Experimental complete eleven-stage managed mutation. The immutable release is independently verified. `executor_state = "integrated-narrow"`, `managed_mutation_support = "narrow-experimental"`, `complete_lifecycle = true` and `platform_support = "none"` remain the authoritative v0.6.0 boundary. The release remains Experimental; the next roadmap milestone is `v0.7.0`.
